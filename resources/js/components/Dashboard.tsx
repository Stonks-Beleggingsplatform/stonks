                    ...
                    <div className="relative z-10">
                        ...
                        <div className="flex items-baseline gap-4">
                            <h1 className="text-5xl font-black tracking-tighter">
                                ${(portfolio[0].total_value || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                            </h1>
                            <div className={`flex items-center text-sm font-bold px-2 py-1 rounded-lg ${(portfolio[0].total_return || 0) >= 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                                <span>{(portfolio[0].total_return || 0) >= 0 ? '↑' : '↓'} {Math.abs(portfolio[0].total_return || 0).toFixed(2)}%</span>
                            </div>
                        </div>
                        <div className="mt-6 flex gap-8">
                            <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Available Cash</p>
                                <p className="text-lg font-bold">${(portfolio[0].cash || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                            </div>
                            <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Invested</p>
                                <p className="text-lg font-bold">${((portfolio[0]?.total_value || 0)).toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                            </div>
                        </div>
                    </div>
                </section>
